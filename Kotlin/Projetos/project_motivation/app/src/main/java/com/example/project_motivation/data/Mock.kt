package com.example.project_motivation.data
...
class Mock {
    ...
    fun getPhrase(value: Int): Phrase {
        val phrasesFiltered = mListPhrase.filter { (it.categoryId == value || value == all) }

        // https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.collections/indices.html
        val randomNumber = (mListPhrase.indices).random()

        return phrasesFiltered[randomNumber]
    }
